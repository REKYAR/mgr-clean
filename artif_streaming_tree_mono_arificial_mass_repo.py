...
output_dir = "mass_stm_monotonicity_experiments_htree"
...
def run_experiment(case_name, constraint_feature=None, constraint_direction=None, 
    ...
    if use_majority:
        ...
        model = HoeffdingTreeClassifierMono(
    ...
    for i, (X, y) in enumerate(main_stream):
        ...
        if use_majority:
            results['max_list_size'].append(len(model.buffer))
            results['avg_list_size'].append(len(model.buffer))
            results['num_lists'].append(1)
        else:
            results['max_list_size'].append(model._n_active_leaves)
            results['avg_list_size'].append(model._n_active_leaves)
            results['num_lists'].append(1)
