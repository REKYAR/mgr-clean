            ...
            for metric, full_metric_key in zip(metrics_to_extract, full_metric_keys):
                if full_metric_key in model_data:
                    if model_data[full_metric_key]:
                        results[model_name][metric] = round(model_data[full_metric_key][-1], 3)
                    else:
...
def main():
    parser = argparse.ArgumentParser(description='Extract final metrics for models and save to CSV')
    parser.add_argument('--file', '-f', type=str, required=True, help='Path to the JSON data file')
    parser.add_argument('--models', '-m', type=str, nargs='+', help='Models to extract metrics for')
    parser.add_argument('--metrics', type=str, nargs='+', default=['accuracy'], 
                        help='Metrics to extract (default: accuracy)')
    parser.add_argument('--output', '-o', type=str, default='model_metrics.csv', 
    ...
    if not args.models:
        print("Error: No models specified. Use --models to specify which models to extract metrics for.")
        list_available_models(data)
        return
