...
class HTLeafMono(Leaf, abc.ABC):
    ...
    def best_split_suggestions(self, criterion, tree) -> list[BranchFactory]:
        ...
        clean_leafs = [
            leaf for leaf in leafs if leaf is not self
        ]  # remove current leaf from leafs
...
class DummySplitter(GaussianSplitter):
    ...
    def build_distributions(
        self, distributions, split_value, side, minpc, maxpc
    ):  # side 0 - lhs, 1 - rhs
...
class CustomGaussianSplitter(GaussianSplitter):
    ...
    ):  # , fresh_fill_strategy=0): #, classes: list[ClfTarget] = None):
    ...
    def _calculate_monotonicity_correction_factor(
        ...
        )  # ordering leaves then simulated leaves in the array order
        ...
        for mc in mcs:
            if mc.feature == att_idx:  # do we have a constraint for this feature
                if mc.monotonicity == 1:
                    val = val and self._is_increasing(
                        leaf1, leaf2
                    )  # 1 when leaf1 nonmonotone wrt leaf2
                    if val == 0:
                        return 1
                else:
                    val = val and self._is_decreasing(
                        leaf1, leaf2
                    )  # 1 when leaf1 nonmonotone wrt leaf2
                    if val == 0:
                        return 1
...
class HoeffdingTreeClassifierMono(HoeffdingTree, base.Classifier):
    ...
    def _attempt_to_split(
        ...
        if not leaf.observed_class_distribution_is_pure():  # type: ignore
            ...
            )  # use monotonicity to influence order of best splits? criterin.merit_of_split calcualtes the merit of the split <- our ordering
