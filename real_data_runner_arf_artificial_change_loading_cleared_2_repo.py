...
class HTLeafMono(Leaf, abc.ABC):
    ...
    def best_split_suggestions(self, criterion, tree) -> list[BranchFactory]:
        ...
        Split candidates.
        """
        leafs = tree._find_leaves() #list of leafs HoeffdingTree._find_leaves
        clean_leafs = [leaf for leaf in leafs if leaf is not self] #remove current leaf from leafs
...
class CustomGaussianSplitter(GaussianSplitter):
    def __init__(self, n_splits: int = 10, monotonicity_constraints: list[MonotonicityConstraint] = [], rho: float = 1e-5, stdevmul: float = 2.0, max_correction_factor: int = 10000):#, fresh_fill_strategy=0): #, classes: list[ClfTarget] = None):
        ...
        self.monotonic_correction_dict={} # ARF DIFF
    ...
    def best_evaluated_split_suggestion(self, criterion, pre_split_dist, att_idx, binary_only, leafs):
        ...
        for split_value in suggested_split_values:
            ...
            if merit > best_suggestion.merit:
                ...
                self.monotonic_correction_dict["greatest"] = self.monotonic_correction_dict["greatest"] if "greatest" in self.monotonic_correction_dict and self.monotonic_correction_dict["greatest"]>correction else correction
...
class HoeffdingTreeClassifierMono(HoeffdingTree, base.Classifier):
    ...
    def _attempt_to_split(self, leaf: HTLeaf | HTLeafMono, parent: DTBranch, parent_branch: int, **kwargs):
        ...
        if not leaf.observed_class_distribution_is_pure():  # type: ignore
            ...
            best_split_suggestions = leaf.best_split_suggestions(split_criterion, self) #use monotonicity to influence order of best splits? criterin.merit_of_split calcualtes the merit of the split <- our ordering
    ...
    def monotonicty_data(self) -> dict:
        ...
        for leaf in leaves:
            if hasattr(leaf, 'splitter') and hasattr(leaf.splitter, 'monotonic_correction_dict'):
                mono_data = leaf.splitter.monotonic_correction_dict
                if "greatest" in mono_data and mono_data["greatest"] > aggregated_data["greatest"]:
                    aggregated_data["greatest"] = mono_data["greatest"]
                if "best_suggestion" in mono_data and mono_data["best_suggestion"] > aggregated_data["best_suggestion"]:
...
class BaseTreeClassifier(HoeffdingTreeClassifierMono):
    ...
    def _new_leaf(self, initial_stats=None, parent=None):
        ...
        if self._leaf_prediction == self._MAJORITY_CLASS:
            return RandomLeafMajorityClassMono( #TODO MAJORITYcLASSMONO
                initial_stats,
                depth,
                self.splitter,
                self.max_features,
...
class ARFClassifierMono(BaseForest, base.Classifier):
    ...
    def __init__(
        ...
        grace_period: int = 200, #from 50
        max_depth: int | None = None,
        split_criterion: str = "info_gain",
        delta: float = 1e-7, #1e-7 - works faster than 1e-3
        tau: float = 0.05,
        leaf_prediction: str = "nba",
        nb_threshold: int = 0,
        nominal_attributes: list | None = None,
        ...
        max_share_to_split: float = 0.99,
        max_size: float = 100.0,
        memory_estimate_period: int = 1_000_000, #used to be 2_000_000
        stop_mem_management: bool = False,
        remove_poor_attrs: bool = False,
        merit_preprune: bool = True,
        seed: int | None = None,
    ...
    def _drift_detector_input(
        ...
        if "best_suggestion" in mono_data:
            monotonicity_violation = int(mono_data["best_suggestion"] > monotonicity_threshold)
        
        weight_pred = 0.7
        weight_mono = 0.3
        combined_signal = weight_pred * prediction_error + weight_mono * monotonicity_violation
...
def run_single_experiment(dataset_name, df, constraints, experiment_name, **kwargs):
    ...
    **kwargs :
    ...
    ht_with_constraints = ARFClassifierMono(
    ...
    ht_without_constraints = ARFClassifierMono(
