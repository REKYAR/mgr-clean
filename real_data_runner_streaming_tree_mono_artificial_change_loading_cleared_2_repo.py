...
def run_single_experiment(dataset_name, df, constraints, experiment_name, **kwargs):
    ...
    **kwargs :
    ...
    for model_key, model in [
        ("with_constraints", ht_with_constraints),
...
def visualize_combined_results(results, plots_dir):
    ...
    bars = plt.bar(
        ...
        model_leaves,
        width=0.6,
        color=plt.cm.viridis(np.linspace(0, 0.8, len(model_keys))),
    )
...
def main():
    ...
    parser.add_argument(
        "--rho", type=float, default=1e-5, help="Monotonicity penalty parameter"
    )
    parser.add_argument(
        "--stdevmul", type=float, default=2.0, help="Standard deviation multiplier"
    )
    parser.add_argument(
        "--grace_period",
        type=int,
