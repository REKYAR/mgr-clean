...
class EnhancedKNNMClassifierV2:
    ...
    def _calculate_replacement_scores(self):
        ...
        for i, (_, _, age, constraint_satisfaction) in enumerate(self.window):
            ...
            constraint_score = (
                1 - constraint_satisfaction
            )  # Higher score for lower satisfaction
                ...
                for f in feature:
                    if f in features.columns:
                        ...
                        monotonicity_constraints.append(
                            {"feature": feature_idx, "monotonicity": 1}
                        )
...
def run_single_constraint_experiment(
    dataset_name, df, constraints, experiment_name, **kwargs
):
    """
    ...
    **kwargs :
    ...
    if hasattr(knnm_with_constraints, "get_stats"):
        ...
        results["models"]["with_constraints"][
            "constraint_violations_avoided"
        ] = stats.get("constraint_violations_avoided", 0)
        results["models"]["with_constraints"][
            "constraint_violations_in_predictions"
        ] = stats.get("constraint_violations_in_predictions", 0)
        results["models"]["with_constraints"][
            "avg_constraint_satisfaction"
        ] = stats.get("avg_constraint_satisfaction", 0)
...
def visualize_combined_results(results, plots_dir):
    ...
    if any(metric in results["models"][model_keys[0]] for metric in violation_metrics):
        ...
        for k in model_keys:
            ...
            if "avg_constraint_satisfaction" in results["models"][k]:
                model_violations.append(
                    results["models"][k]["avg_constraint_satisfaction"] * 100
                )
            else:
                model_violations.append(0)
        ...
        metrics_labels = [
            "Violations Avoided",
            "Violations in Predictions",
            "Avg. Constraint Satisfaction (%)",
        ]

        for i, (metric_name, metric_label) in enumerate(
            zip(violation_metrics + ["avg_constraint_satisfaction"], metrics_labels)
        ):
            values = [data[i] for data in violation_data]

            plt.bar(
                x + i * bar_width - bar_width, values, bar_width, label=metric_label
            )
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(plots_dir, f"combined_constraint_stats_{dataset_name}.png")
        )
        plt.close()
...
def run_all_experiments(
    ...
    for name, (df, constraints) in datasets.items():
        ...
        minimum_new_sample_quota = min(
            0.5, max(0.05, min_constraint_size / window_size)
        )
    ...
    if iris_datasets:
        ...
        for model_key, model_data in results["models"].items():
            ...
            if "constraint_violations_avoided" in model_data:
                print(
                    f"    Constraint violations avoided: {model_data['constraint_violations_avoided']}"
                )

            if "avg_constraint_satisfaction" in model_data:
                print(
                    f"    Avg constraint satisfaction: {model_data['avg_constraint_satisfaction']:.4f}"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run monotonicity constraint experiments on UCI datasets"
    )
    parser.add_argument(
        "--data_dir",
        type=str,
    ...
    parser.add_argument(
        "--window_size", type=int, default=100, help="Window size for KNN"
    )
    parser.add_argument(
        "--min_constraint_size",
        type=int,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="balanced_results_multi_v2",
