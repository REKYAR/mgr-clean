...
class TreeBaggingSamplingClassifier:
        ...
        for _ in range(self.nr_clf_to_probe):
            ...
            if X_test:
                ...
                if X_train_np.size > 0:
                    ...
                    score = accuracy_score(y_train_np, y_train_pred) # Fallback to training accuracy
                else:
                    score = 0 # Or handle as an error/skip

            if self.monotonicity_constraints:
                monotonicity = -1 * self.evaluate_tree_monotonicity(model)
                score += monotonicity #add monotonicity to score to promote monotonic trees
    ...
    def evaluate_tree_monotonicity(self, tree: DecisionTreeClassifier):
        ...
        minmax_values = np.full((n_nodes, self.feature_cnt, 2), np.inf) # node amt, feature amt, min/max
        ...
        for i in range(n_nodes):
            for j in range(n_nodes):
                if i==j and is_leaves[i]:
                    matrix[:, i, j] = 0 #setting all features to 0 for comparing leaf to itself should be ok
                    continue
                for constraint in self.monotonicity_constraints:#foreach constraint check monotonicity
                    ...
                    if is_leaves[i] and is_leaves[j]:
                        if monotonicity == 1: #increasing
                            if arr[i] > arr[j] and minmax_values[i,feature_id,0] < minmax_values[j,feature_id,1]: #class i > class j and min i < max j
        ...
        for constraint in self.monotonicity_constraints:
            ...
            feature_mtx = np.where(feature_mtx == None, 0, feature_mtx) #convert all Nones to 0 because it does not impact W
...
def run_single_experiment(dataset_name, df, constraints, experiment_name, **kwargs):
    ...
    **kwargs :
    ...
    grace_period = kwargs.get('grace_period', 200) # This is fetched from kwargs, primarily for record-keeping in results
    max_depth = kwargs.get('max_depth', None) # This is fetched from kwargs, primarily for record-keeping in results
    ...
    minibatch_size = kwargs.get('minibatch_size', 50) # This is the minibatch_size for TreeBaggingSamplingClassifier.
