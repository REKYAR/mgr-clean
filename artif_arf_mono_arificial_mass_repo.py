...
class CustomGaussianSplitter(GaussianSplitter):
    def __init__(self, n_splits: int = 10, monotonicity_constraints: list[MonotonicityConstraint] = [], rho: float = 1e-5, stdevmul: float = 2.0, max_correction_factor: int = 10000):#, fresh_fill_strategy=0):
    ...
    def _is_increasing(self, leaf1, leaf2):
        if len(leaf1.splitter._att_dist_per_class) == 0 or len(leaf2.splitter._att_dist_per_class) == 0:
            return 0
